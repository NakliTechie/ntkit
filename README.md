...
| `/resume-nt` | start of session | Read the handoff (workplan + pending + history + latest summary + git state), name the repo's **state** and its legal next moves, flag anything that doesn't add up, and wait. Read-only. |
...
| `/execute-nt` | anytime | Work a batched fix-workplan from a `/forward-pass-nt` / `/ux-review-nt` / `/maintain-nt` report — fix + verify each item, check it off with a **what · evidence · result** progress row (SHA / `file:line` / check output — pointers that resolve, not prose), pause between batches. Refuses to run without an open plan. The **executor** for static findings. |
