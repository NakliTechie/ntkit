...
| `/windup-nt` | end of session | Day summary + pending + workplan + push + tomorrow's handoff. Honest about the closing state — a mid-chunk close gets named, never papered over. Runs an **implicit `/replan-nt`** first when `plan/` has accumulated 3+ foldable files, so consolidation is ambient, not a chore to remember. |
