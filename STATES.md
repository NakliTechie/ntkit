...
| `building` | `/decide-nt`, `/soc-nt`, `/windup-nt` (warns), `/autopilot-nt`, `/siege-nt` (if a live instance is reachable) | `/release-nt`, `/package-nt` |
| `verifying` | `/live-check-nt` (the replay gate), `/siege-nt` (iterated, before a live-facing surface is called ready), `/autopilot-nt`, `/walkthrough-nt`, `/forward-pass-nt` | `/release-nt` with open items |
