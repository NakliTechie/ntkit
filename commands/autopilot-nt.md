...
Per-item checks prove each fix in isolation; they don't prove the fixes coexist — an early item can break a later one's assumptions while every individual check stays green. So before writing the report, run the **whole-project deterministic gate once**: the full test suite, the typecheck, the build, the linter — whatever the project has. Green → the run is **landed**. Red → the run is **not landed**, whatever the per-item log says: bisect if cheap (revert the most recent suspect commits on the branch until green, park what you reverted), otherwise report the branch red with the failure output front and center. "Done" is the gate's word, never the agent's.
...
Landed (fresh-eyes verified, committed — unmerged, unpushed):
...
Left on the branch: <N commits>. Nothing pushed, sent, merged, or deleted.
Review: git diff main...autopilot/<date> — merge or discard, then git worktree remove .worktrees/<name>
Resume: cd <main checkout absolute path> and run /resume-nt (it reads this report)
```

The three lines that matter most are the **Final gate** (whether the branch is trustworthy at all), **Landed** (with SHAs, so it's auditable), and **Needs you** (so the human's scarce attention goes straight to what only they can unblock). Everything reversible was decided and logged; nothing outward was touched. Finish by firing `/notify-nt "<project>: autopilot done — gate <GREEN|RED>, <N> landed, <M> need you"` so the finish is a ping, not a surprise — it degrades silently if no channel is configured. `/resume-nt` reads this report straight into the next session.
