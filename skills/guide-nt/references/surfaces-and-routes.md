...
- **The hero flow** — pick the 2–3 captures that show the product doing its job: **entry → key action → result** (open the app, do the one thing it exists for, see the outcome). Take them from one role, the primary user's, in order. Record them as `HERO_FLOW`, a list of capture ids (`<role>/NN-<slug>`), next to the route-plans. It is the storyboard `/package-nt` hands to `/brag-slim` for the launch video, so the video shows real screens with seeded data instead of rebuilt ones. A landing page describing the product is not a hero step; the working app is. Keep an existing `HERO_FLOW` unless a route it names was removed or renamed.
