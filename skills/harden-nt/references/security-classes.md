...
A generic adversarial brief — "find a case this surface does not handle
correctly" — leaves an agent staring at a blank page. When the surface being
hardened is security-sensitive (takes untrusted input over a network, holds
another user's data, runs an LLM agent or MCP server, or ships to
Cloudflare/cloud infra), hand the adversarial round a slice of this file
instead of the bare brief — one class or domain section per agent, contract
only, no hints toward which one is actually weak. Skip this file for a surface
with no external input; the generic brief is enough there.
...
Each class/domain bullet is one adversarial agent's brief: contract + this
section, nothing else. A finding it returns is a path the map forgot — add it
per Phase 5, then run it through the same constructive → harden → verify
sequence as any other path. A candidate that needs a live dependency, provider
config, or browser you can't reach from this surface is not yet a path: name
the missing fact and leave it uncovered rather than guessing either way.
