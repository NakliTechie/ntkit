...
Also settled here, stated not asked (veto-window style, per the STATES.md ask rule): the **critic cadence** (default: every 5 experiments), the **stagnation threshold** (default: 8 consecutive experiments without improvement ends the leg STAGNANT), and the **prior-art check** — before any experiment runs, sweep the vault (`/ask-nt`, from scholia, when installed) **first**, then the repo and its `plan/`, then the web — the vault is curated and carries your own conclusions, so it answers in one command what a search answers in ten, and sometimes tells you the consensus is wrong for your case; a question already answered ends the campaign at leg zero, GOAL-MET by citation, and that's the cheapest win the command can deliver.
