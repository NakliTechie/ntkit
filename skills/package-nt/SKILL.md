...
- **Social card on both surfaces the project has: the repo, and the deployed app.** Two different URLs get shared, and a card fixed on one leaves the other showing nothing.
  - **Repo:** `gh api graphql -f query='{repository(owner:"<owner>",name:"<repo>"){usesCustomOpenGraphImage}}'` must be `true` — `false` means every shared repo link shows GitHub's default card → **blocker**. `true` alone doesn't mean the image is *current* (no hash/date is exposed); if `marketing/social.png` (or the repo's equivalent) changed since the last upload, re-upload it. There is no API to set it — [`references/social-preview.md`](references/social-preview.md) has the exact drive-the-browser procedure (Claude-in-Chrome, not the built-in pane; one specific step must not be a click) and how to confirm it actually persisted, not just rendered once. Also check the repo's **"About" description** (`gh repo edit --description`) is current — it feeds `og:title`/`og:description` and is a separate field the README rebuild doesn't touch.
