...
Whichever ran, the report header carries one line — `Reviewer: <model> · prior: <model> (<date>)` or `prior: none` — that line is the whole log; the next run reads it to rotate. If you are the family that built most of this code, say that there too — the reader should know which grade of eyes graded it. That self-grade is **unconditional**: it applies on a first run with no prior report and on a run where no rotation was available, not only when eyes actually changed.
