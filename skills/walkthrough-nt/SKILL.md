...
**Rotate the eyes.** This is a checker command; two runs of the same model misjudge the same things identically. Before starting, read only the `Reviewer:` header line of this command's most recent report in `plan/`. **From a worktree, `plan/` is gitignored and therefore absent** — the prior report lives in the main checkout, so look there before concluding `prior: none`. Repeat run → prefer a different **model family** (an agent CLI on the PATH such as `codex` or `gemini`, checked with `which`, driven through this same brief via Bash); none reachable → a different model of the current family (subagent with a model override); neither → run as-is and say so in the report. Never rotate to materially weaker eyes; log the rotation as unavailable instead. The report header carries `Reviewer: <model> · prior: <model> (<date>)` or `prior: none`, and names it if you are the family that built most of this code — **that self-grade is unconditional**, applying on a first run with no prior report and on a run where no rotation was available. On a first run there is nothing to rotate *from*, so rotating is optional; the builder-family disclosure is not.
...
| 5 Chaos leg | A bounded random walk from the states the scripted journey already reached, with the Phase 3 invariants as the oracle — the space a written journey cannot cover by construction. Driven from a **logged seed** (without one, replay is unimplementable). Every breach replayed from the action log before it earns an ID; confirmed ones fixed under Phase 4's rules and pinned as regression cases. Skippable, never silently. | `references/chaos.md` |
