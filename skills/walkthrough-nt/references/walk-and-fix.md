...
3. **If it broke, fix it now.** First give it a stable ID by severity — `C1/H1/M1/L1` (Critical / High / Medium / Low), the same scheme `/forward-pass-nt` uses so the IDs read the same downstream — and capture evidence: repro steps · expected vs. observed · a console/network excerpt · a screenshot. Then close the loop:
   - **Check your assertion before you check your environment — your driver is a suspect, and the suspect is usually the assertion.** Don't fix a symptom you can't trigger, and don't fix one you triggered only through your own instrumentation. A harness written minutes ago fails in ways that look exactly like application bugs: a re-entrant input loop interleaves characters and reads as data corruption; a race between a screenshot and a re-render reads as a rendering bug; a programmatic `.click()` doesn't move focus the way a real click does, so focus-restore looks broken when it isn't; state read synchronously after an `async` handler is read before the handler resumed, so persistence looks broken when it isn't. Two checks, and **the second one catches most of them**:
     ...
     The same skepticism applies to your *tools'* silence, not just your assertions. A `grep` that finds nothing in a file containing a NUL byte prints nothing and exits 1 — indistinguishable from "not present" — which is how one run nearly rebuilt a handler the file already had. `rg` reports `binary file matches` and names the offset; `grep -a` reads it as text. **A search miss is not evidence of absence.**

     3. **Confirm your edit landed where you meant.** A fix that appears not to work is often a fix that was never applied to the thing you think it was. A string replace matching an earlier occurrence, an edit to the source of a generated file without running the generator, a change in the wrong worktree — each produces a red that looks exactly like "my fix is wrong." Before concluding a fix failed, read back the changed line and confirm it is the one you targeted. (One dogfood run put a CSS property on the first rule in the file rather than the intended one, and read the resulting red as the fix being wrong.)

     Still broken after all three → real. **This command edits code, so the cost of skipping this is a commit that changes working code in a shipped product to chase an artifact you created.** One run was a single probe away from rewiring a working handler.
   ...
   - **Prove which change closed it, when you made more than one.** Two edits and one green tells you nothing about which mattered. Run the isolation matrix — each change alone, then both — and read it. One dogfood run found its product fix closed both failures on its own, while its *test-harness* change alone also went green on one of them **for the wrong reason**: a shorter wait moved the key event earlier relative to the race, leaving the defect latent under a passing check. Without the matrix it would have shipped that as the fix.
